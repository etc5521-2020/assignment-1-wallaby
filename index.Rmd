...
ow they expand on what has been done already, why these would be interesting to pursue, and how it broadens the scope of the original analysis.
analysing the trend of music characeteirstics overtime is an interesting thing to look at as
and as previous team has analysed the correlation of music characteristics, i think it would be beneficial to also look at how the music characteristics are changing over time. As music evolves, we wanna look at how the characteristics evolve. Are the music characteristics in 1957 similar to those in 2019? 


To answer the research question "How is the music characteristic over time and how is it changing?", we 
```{r}
characteristics_mean <- spotify_songs %>%
 select(danceability,
        ...
        valence,
        year) %>%
  pivot_longer(-year, names_to = "characteristics",
               values_to = "values") %>%
  group_by(year,
           characteristics) %>%
...
Now, the music characteristics might change because of the emerging of new types and more modern music. But is the music characteristic also changing for an artist?
it would be interesting to look at the trend to see if one specific artist also has a shift in his music characteristics. Is the same artist making the same kind of music through time?
 ...
 ggplot() +
  ...
  facet_wrap(~characteristics, scales = "free") +
   theme(legend.position = "none",
         ...
         axis.title.y = element_text("Values"),
         plot.title = element_text("Trend of Music Components Overtime"))
```
