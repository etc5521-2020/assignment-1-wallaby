...
```{r my-favorate-tracks, eval = FALSE}
get_my_top_artists_or_tracks(type = 'tracks', time_range = 'long_term', limit = 5) %>% 
  select(name, album.release_date) %>% 
  rowwise %>% 
  write_csv("my_top5_tracks.csv")
# Save the result as a csv file because the authorization to my spotify account may fail when runing on other devices. 
```

```{r my-top5-track}
read_csv(here::here("my_top5_tracks.csv")) %>% 
...
```{r my-artists-recently, eval=FALSE}
...
my_artists %>% 
  ...
  write_csv("my-recent-artists.csv")

```

```{r my-recent-artists-popular}
read_csv(here::here("my-recent-artists.csv")) %>% 
...
```{r my-recent-artists-niche}
read_csv(here::here("my-recent-artists.csv")) %>% 
  arrange(popularity) %>% 
  head(5) %>% 
  kable(caption = "The niche artists in my playlist", booktabs = T) %>% 
  kable_styling(latex_options = c("striped", "hold_position"))
```
