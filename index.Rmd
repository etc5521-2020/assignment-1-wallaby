...
By doing this exploratory data analysis, we want to konw: 
...
2. Explore our favorate artist - Coldplay's works, e.g. how about the musical positiveness conveyed by their albums?
...
```{r radarplot, fig.cap="Characteristics of top singers", fig.align='center', fig.height=5}
...
## Music genre and their popularities - by decade of release date
...
duration_popular <- spotify_songs %>% 
  ggplot() + 
  geom_point(aes(x = (duration_ms / 100), y = track_popularity), color = "#C3D7A4", alpha = 0.05)
...
Also, We are not sure whether those above dot plots can directly reveal the relationship between these popularity and audio features. So we pay attention to exploring whether these audio features contribute to a higher popularity using a linear regression model just in case. And the following table shows all the audio features with a p-value less than 0.05. We can draw a conclusion that danceability contributes most to a higher popularity, loudness has a positive relationship with popularity and liveness has a negative relationship with popularity, with is the same with those dot plots conclusion.
...
First of all, there is a positive or negative correlation between audio features and track popularity. However, the relationship coefficient is not very large. The popularity of music artworks depends more on the artist's own popularity, creative talent or singing ability, or external factors such as world trends. The probability of success by deliberately catering to audio features and creating specific songs is not sufficient. 
